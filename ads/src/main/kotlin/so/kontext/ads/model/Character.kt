 ...
 *
 * `avatarUrl` is typed as [java.net.URI] (not `String`) so malformed
 * URLs fail at construction. `URI` is preferred over `java.net.URL`
 * (whose `equals` does DNS resolution) and over `android.net.Uri`
 * (which requires Robolectric for unit tests). Wire encoding goes
 * through [CharacterDto], where the URI is serialized via `toString()`.
 *
